...
class GameData {
  ...
  Future<void> loadData(String root) async {
    ...
    final String response =
        await rootBundle.loadString('${root}summons.json', cache: false);
    ...
    final String editions = await rootBundle
        .loadString('${root}editions/editions.json', cache: false);
  ...
  Future<EditionRoomsModel?> fetchRoomData(String campaign, String root) async {
    try {
      final String response =
          await rootBundle.loadString('${root}rooms/$campaign.json');
      final data = await json.decode(response);
  ...
  fetchCampaignData(String campaign, String root,
    ...
    final String response = await rootBundle
        .loadString('${root}editions/$campaign.json', cache: false);
    final data = await json.decode(response);
