...
void main() {
  ...
  group('ChangeStatCommand (via ChangeHealthCommand)', () {
    test('handleDeath removes monster instance when health reaches 0', () {
      ...
      // Deal lethal damage
      ChangeHealthCommand(-currentHp, monsterInstance.getId(), monster.id, gameState: getIt<GameState>())
          .execute();
    ...
    test('handleDeath keeps character in list when health reaches 0', () {
      ...
      ChangeHealthCommand(-currentHp, character.id, character.id, gameState: getIt<GameState>()).execute();
    ...
    test('describe returns "change stat"', () {
      ...
      final command =
          ChangeHealthCommand(1, monsterInstance.getId(), monster.id, gameState: getIt<GameState>());
    ...
    test('handleDeath: character un-death triggers update (health 0 → positive)', () {
      ...
      ChangeHealthCommand(-maxHp, character.id, character.id, gameState: getIt<GameState>()).execute();
      expect(character.characterState.health.value, 0);
      // Revive: going from 0 to positive triggers the un-death path (line 23)
      expect(
        () => ChangeHealthCommand(1, character.id, character.id, gameState: getIt<GameState>()).execute(),
        returnsNormally,
      );
    ...
    test('handleDeath: summon death removes summon from character summonList', () {
      ...
      AddStandeeCommand(1, summonData, bannerSpear.id, MonsterType.summon, true, gameState: getIt<GameState>())
          .execute();
      ...
      ChangeHealthCommand(-1, summonId, bannerSpear.id, gameState: getIt<GameState>()).execute();
    ...
    test('handleDeath: summon death when more summons remain still updates', () {
      ...
      AddStandeeCommand(1, summonData1, bannerSpear.id, MonsterType.summon, true, gameState: getIt<GameState>())
          .execute();
      AddStandeeCommand(2, summonData2, bannerSpear.id, MonsterType.summon, true, gameState: getIt<GameState>())
          .execute();
      ...
      ChangeHealthCommand(-1, firstSummonId, bannerSpear.id, gameState: getIt<GameState>()).execute();
