...
void main() {
  ...
  group('SetScenarioCommand character state reset', () {
    ...
    test('character xp is reset to 0', () {
      ...
      final after =
          getIt<GameState>().currentList.firstWhere(
                (e) =>
                    e is Character &&
                    !GameMethods.isObjectiveOrEscort(e.characterClass),
              )
              as Character;
    ...
    test('objectives from previous scenario are removed on new scenario', () {
      ...
      expect(
        getIt<GameState>().currentList.any(
          (item) =>
              item is Character &&
    ...
    test('previous scenario monsters are replaced on new scenario', () {
      ...
      // #5 has Zealot
      expect(
        getIt<GameState>().currentList.whereType<Monster>().any(
          (m) => m.id == 'Zealot',
        ),
        isTrue,
      );
    ...
    test('previous ability decks are replaced on new scenario', () {
      ...
      final decksAfterFirst = getIt<GameState>().currentAbilityDecks
          .map((d) => d.name)
          .toSet();
      ...
      final decksAfterSecond = getIt<GameState>().currentAbilityDecks
          .map((d) => d.name)
          .toSet();
  ...
  group('SetScenarioCommand ally rules', () {
    ...
    test('monsters listed in Allies rule are marked isAlly', () {
      ...
      final ratMonstrosity = getIt<GameState>().currentList
          .whereType<Monster>()
    ...
    test('monsters not in Allies rule are not marked isAlly', () {
      ...
      final blackSludge = getIt<GameState>().currentList
          .whereType<Monster>()
    ...
    test(
      'shouldShowAlliesDeck returns true when allied monsters are present',
      () {
        SetScenarioCommand(
          '#6 Corrupted Research',
          false,
    ...
    test('showAllyDeck flag stays false when allies come from "Allies" rule '
    ...
    test(
      'showAllyDeck is reset to false when loading a scenario without AllyDeck rule',
      () {
  ...
  group('SetScenarioCommand Objective special rule', () {
    test('all 7 objectives from #6 Corrupted Research are added to list', () {
      ...
      final objectives = getIt<GameState>().currentList.where(
        (item) =>
            item is Character &&
            GameMethods.isObjectiveOrEscort(item.characterClass),
      );
    ...
    test('objective display names match scenario data (Growth 1–7)', () {
      ...
      final names = getIt<GameState>().currentList
          .whereType<Character>()
    ...
    test('objectives have positive health', () {
      ...
      for (final obj
          in getIt<GameState>().currentList.whereType<Character>().where(
            (c) => GameMethods.isObjectiveOrEscort(c.characterClass),
          )) {
  ...
  group('SetScenarioCommand sections', () {
    ...
    test('adding the same section twice records it twice', () {
      ...
      expect(
        getIt<GameState>().scenarioSectionsAdded
    ...
    test(
      'scenarioSectionsAdded is cleared when a new base scenario is loaded',
      () {
        SetScenarioCommand(
          sectionName,
          true,
    ...
    test('adding a section does not remove previously-added monsters', () {
      ...
      final monstersBefore = getIt<GameState>().currentList
          .whereType<Monster>()
          .length;
