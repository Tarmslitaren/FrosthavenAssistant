...
void main() {
  ...
  group('RemoveCSPartyCardCommand', () {
    test('should remove party card from character modifier deck', () {
      AddCSPartyCardCommand(character.id, 1, gameState: getIt<GameState>()).execute();
      final deck = character.characterState.modifierDeck;
      expect(deck.hasCard('party/1'), isTrue);

      RemoveCSPartyCardCommand(character.id, gameState: getIt<GameState>()).execute();
    ...
    test('describe includes character id', () {
      final command = RemoveCSPartyCardCommand(character.id, gameState: getIt<GameState>());
